...
3. Do a MC for when things are better
4. Do not care about faults, but care about size and weight - just additional module - like on a ground robot

### Bonus AHP stuff which could be used:
* [ ] Does the system go into swap
* [ ] The entire timing stuff, which we neglect for this - enough literature there adressed
* [ ] Process-wise monitoring

## 3 - Monte- Carlo Simulation
### Simulation 1
to generate matrices, which can be chosen for the AHP and are consistent. Do 10000 iterations for matrices of rank 2,3,4. 
2 only gives 17 options (the 17 options that we have available from the original scale.)
for 3 and 4 we have roughly 11.000 and 400 values respectively (because of consistency checks - harder to achieve for rnk=4)

### Simulation 2
Use the numbers generated by the preliminary tests for relative importance. Use the matrices generated from the first monte-carlo simulation.
Choose a random matrix from the possible matrices and calculate it through. Calculate the end result and write it down in a big array.

### Processing of MC
1. [ ] Find the index, column pair of the maximum value
2. [ ] Get the relative AHPs out
