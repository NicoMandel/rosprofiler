...
class Timinglogger:
    ...
    def __init__(self, topic, metrics, window_size=1000, filter_expr=None, timer_freq=0.5, filename="freqfile"):
        ...
        rospy.Subscriber(topic, rospy.AnyMsg, rthz.callback_hz)
        ...
        rospy.Subscriber(topic, rospy.AnyMsg, rtbw.callback)
        ...
        self.logging_df = pd.DataFrame(columns=self.Metrics)
    ...
    def timer_callback(self, event): # has to take a rospy.TimerEvent, see documentation
        ...
        temp_df.at[t, "HZ_Mean"] = pd.np.mean(hz_vals)
        temp_df.at[t, "HZ_Max"] = pd.np.max(hz_vals)
        temp_df.at[t, "HZ_Min"] = pd.np.min(hz_vals)
        temp_df.at[t, "HZ_Std Dev"] = pd.np.std(hz_vals)
...
class LoggerList:
    ...
    def __init__(self, list_of_topics, filename="topics"):
        ...
        self.metrics = ["Time", "HZ_Samples", "HZ_Mean", "HZ_Max", "HZ_Min", "HZ_StdDev", "BW_Samples", "BW_Bytes / sec", "BW_Mean", "BW_Max", "BW_Min"]
        self.loglist = []
        for topic in list_of_topics:
            loglist.append(Timinglogger(topic, self.metrics))
